...
median :: [Int] -> Int
median arr = if even arrLength 
        then ((middle + middleNext) `div` 2)
        else  middle
        where
                arrLength = length arr
                sorted = sort' arr
                middle = sorted !! (arrLength `div` 2)
                middleNext = sorted !! (arrLength `div` 2 + 1)
...
meanTSValues = meanTS (diffTS tsAll)
